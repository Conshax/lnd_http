...
impl Client {
    ...
    pub async fn send_keysend(&self, dest_hex: &str, amt_msat: u64, mut dest_custom_records_slice: HashMap<u64, &[u8]>, outgoing_chan_id: Option<u64>) -> Result<[u8; 32]> {
        ...
        let dest_custom_records_b64 = dest_custom_records_slice.into_iter().map(|(k, v)| (k, slice_to_b64(v))).collect();
...
mod tests {
    ...
    async fn test_send_keysend() {
        ...
        let res = client.send_keysend(&destination, 1000, custom_records, None).await;
